...
export async function GET() {
  try {
    ...
    const payments = await db.payment.findMany({
      ...
      select: { amount: true, agencyFee: true, createdAt: true, booking: { select: { listing: { select: { title: true } } } } },
      orderBy: { createdAt: "desc" },
    });
    const totalRent = payments.reduce((sum, payment) => sum + payment.amount, 0);
    const platformFees = payments.reduce((sum, payment) => sum + payment.agencyFee, 0);
